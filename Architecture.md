...
edge-triggered PEAK detector (per-chunk peaks must stay below
`ARM_SIGNAL_THRESHOLD_DB`, default -20 dBFS, for ~1 s, then the first
chunk at or above it fires). Peak rather than smoothed RMS because the
trigger event is the needle set-down thump — a sharp transient that
registers on any record however quiet the music — while runout-groove
clicks (~-29 dBFS) stay safely below the boundary, so the post-side
runout can't re-trigger. On fire, the arm thread calls the same `_start_recording_impl`
...
| `ARM_SIGNAL_THRESHOLD_DB`    | `-20`            | arm trigger peak threshold (dBFS)    |
