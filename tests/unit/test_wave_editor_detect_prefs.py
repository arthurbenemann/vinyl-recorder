"""Pytest runner for `wave_editor_detect_prefs.test.js`.
...
TEST_FILE = REPO_ROOT / "tests" / "unit" / "wave_editor_detect_prefs.test.js"


@pytest.mark.skipif(shutil.which("node") is None, reason="node not on PATH")
def test_we_detect_setting_value_via_node():
    r = subprocess.run(
        ["node", str(TEST_FILE)],
