...
def _detector(**kw) -> ArmDetector:
    defaults = dict(threshold_int=1000, bytes_per_sample=BYTES_PER_SAMPLE,
                    bytes_per_second=BYTES_PER_SECOND, quiet_seconds=2.5)
    defaults.update(kw)
...
def test_fires_on_silence_then_signal():
    det = _detector()
    assert not _feed(det, 0, 3.0)        # quiet-confirm (2.5 s default)
    assert det.ready
    assert det.update(_chunk(20000, 0.05))   # one loud chunk is enough


def test_needle_drop_click_fires():
    """The point of peak detection: a single short set-down transient
...
def test_runout_clicks_can_never_re_ready_the_detector():
    """Runout-groove clicks peak ABOVE the shared threshold (~-29 dBFS vs
    -40) roughly every 1.8 s — shorter than the 2.5 s quiet-confirm — so a
    side circling in the runout after auto-stop keeps resetting the clock
    and the arm can never fire on runout noise."""
    det = _detector()
    fired = False
    for _ in range(10):                  # ~19 s of runout
        fired = _feed(det, 0, 1.8) or fired           # surface noise
        fired = det.update(_chunk(2000, 0.05)) or fired   # revolution click
...
def test_signal_resets_quiet_confirm_clock():
    """Quiet must be CONTINUOUS: 1.5 s quiet + above-threshold blip +
    1.5 s quiet is not 2.5 s of quiet."""
    det = _detector()
    _feed(det, 0, 1.5)
    det.update(_chunk(20000, 0.1))       # blip resets the clock
    _feed(det, 0, 1.5)
    assert not det.ready
    _feed(det, 0, 1.1)                   # 2.6 s since the blip
    assert det.ready
...
def test_refires_after_quiet_gap():
    """Side A ends, needle lifted (continuous quiet), record flipped,
    needle drops → re-fire. This is the hands-free multi-side loop."""
    det = _detector()
    _feed(det, 0, 3.0)
    assert _feed(det, 20000, 0.3)        # side A trigger
    _feed(det, 0, 3.0)                   # needle lifted / flipping
    assert det.ready
